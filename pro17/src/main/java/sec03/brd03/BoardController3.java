	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(action ==null) {
				...
				pw.print("<script>"+"alert('새글 추가했다~~');"+"location.href='"+ request.getContextPath() + "/board3/listArticles.do';"+"</script>" );
				return;
				...
				PrintWriter pw =response.getWriter();
				pw.print("<script>"+ " alert('답글 달렸다');"+"location.href='"+request.getContextPath()+"/board3/viewArticle.do?articleNO="+articleNO+"';"+"</script>");
				return;
			}else {
				articlesList =boardService.listArticles();
				request.setAttribute("articlesList", articlesList);
